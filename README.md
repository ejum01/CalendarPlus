...
---
- **느낌 점**
이번 프로젝트를 진행하면서 가장 크게 느낀 점은 설계의 중요성이었습니다. 처음에는 기능 구현에만 집중했지만, 진행할수록 구조와 설계가 얼마나 중요한지 절실히 깨달았습니다. 명확한 요구사항 분석과 그에 따른 아키텍처 설계가 부족하면, 이후 과정에서 많은 수정이 필요하게 되고, 그만큼 비효율적이 될 수 있다는 점을 몸소 체험했습니다.

특히 일정 관리 기능을 구현하면서 예외 처리에 대한 고민이 많았습니다. 사용자가 실수하거나 예상치 못한 상황이 발생할 때, 어떻게 오류를 처리하고 사용자에게 적절한 메시지를 전달할지에 대한 부분은 서비스의 신뢰성에도 큰 영향을 미친다는 것을 느꼈습니다. 예외 상황을 미리 예측하고, 유연하게 처리하는 방법을 고민하는 과정에서 많은 배움이 있었습니다.

이번 프로젝트에서 크게 배운 세 가지를 정리하자면:

API 설계의 중요성: API가 명확하고 일관되게 설계되어야, 사용자 경험이 향상되고 시스템의 유지보수도 쉬워진다는 것을 깨달았습니다.
테스트의 필요성: 단순히 코드를 작성하는 것뿐만 아니라, 다양한 상황을 가정하고 테스트 케이스를 작성하는 것이 얼마나 중요한지 알게 되었습니다.
예외 처리: 오류 메시지를 사용자에게 어떻게 전달할지, 그리고 시스템 내부에서 오류를 어떻게 처리할지에 대해 더 깊이 고민할 수 있는 시간이었습니다.
마지막으로, 이번 프로젝트는 저에게 개발자로서 한 단계 더 성장할 수 있는 좋은 기회였습니다. 더 나은 코드를 작성하고, 더 견고한 시스템을 만드는 방법을 고민하게 되었고, 앞으로도 이런 과정을 통해 지속적으로 성장하고 싶습니다.
